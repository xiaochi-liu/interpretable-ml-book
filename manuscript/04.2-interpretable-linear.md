...
Also if the weather was misty, the expected number of bikes was -379.4 lower, compared to good weather, given all other features stay the same.
...
For example, for the value WORKING DAY of feature workingday, the effect is 124.9209381.
...
When we average the predictions for the training data instances, we get a mean value of 4504 (). 
In comparison, the prediction of the 6-th instance is small with only 1571 predicted bicycles.
...
![With increasing penalty of the weights, fewer and fewer features receive a non-zero weight estimate. These curves are also called regularization paths.](images/llasso-path-1.png)
