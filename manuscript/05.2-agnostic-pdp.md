...
![Partial dependence plots for the bike count prediction model and the season. Unexpectedly all seasons show the same effect, only for spring the model predicts less bicycle rentals.](images/pdp-bike-cat-1.png)
...
![Partial dependence plots of cancer probability based on age and years with hormonal contraceptives. For age, the partial dependence plot shows that the probability is low until 40 and increases after. The more years on hormonal contraceptives the higher the predicted cancer risk, especially after 10 years. For both features not many data points were available for large feature values, so the partial dependence estimates are less reliable in those regions.](images/pdp-cervical-1.png)
